...
struct vertice{							// struct que define um vértice, com letra, descrição e prazo
	char      letra,					// string letra é auxiliar para armazenar as tarefas a serem adicionados no grafo
		   desc[39],					// string desc é auxiliar para armazenar as descrições das tarefas a serem adicionados no grafo  
 		   anterior;					// caractere que guarda o vértice precedente; isso será importante para a escrita do caminho crítico, após os cálculos dos TMT
...
int main(){
	...
	printf("\t\t----------PERT - Program Evaluation and Review Tecnique----------\n");
	...
	if (hasCycle(&gr, &p, &begin, &s)){									// chama a função que verifica se há ciclo
		...
		for(i = begin; i <= p.topo; i++){								// imprime os elementos do "início" até o topo da pilha
			fprintf(out, "%c ", gr.ver[p.vet[i]].letra);							
		}
		...
		if ( gr.v != 0) {												// caso grafo sem vértices nada é mostrado
		...
		if ( gr.v != 0) {											// caso grafo sem vértices nada é mostrado
			...
			for(i = s.topo; i >= 1; i--){											// percorre os vértices do grafo em ordem topológica
				max = gr.TMT[s.vet[1]];												// atribui ao valor máximo o TMT que tem todos os outros vértices como anteriores na ordem topológica
				for (j = 1; j <= s.topo; j++){										// percorre a quantidade de vértices, para que se possa "varrer" a matriz de adjacência
					if(gr.adj[s.vet[j]][s.vet[i]] == 1){							// analisar somente os precendentes
						if (max < gr.TMT[s.vet[j]]){								// se encontrar um TMT maior que o máximo, então ele será o novo máximo
							max = gr.TMT[s.vet[j]];									
							gr.ver[s.vet[i]].anterior = gr.ver[s.vet[j]].letra;		// guarda o predecessor de maior TMT para cada vértice
						}
					}
				}
				if (gr.ver[s.vet[i]].anterior != '.')								// apenas para quem tem precedentes
					gr.TMT[s.vet[i]] = gr.ver[s.vet[i]].prazo + max;				// calcula o TMT de um vértice com base no TMT de vértices anteriores
			}
			
				// Basta agora descobrir o máximo TMT calculado, e esse será o TMT do projeto como um todo
			max = gr.TMT[1];						// atribui o valor máximo o TMT de um vértice qualquer
			fim = 1;								// atribui o valor inicial de fim
			for (i = 1; i <= gr.v; i++){			// varre todos os vértices
				if (max < gr.TMT[i]){				// se encontrar um TMT maior que o máximo, então ele será o novo máximo
					max = gr.TMT[i];
						fim = i;					// guarda o novo índice do vértice final do caminho crítico
				}
			}
			
			inicializaPilha(&p);						// inicializa a pilha de predecessores
			empilha(&p, fim);							// coloca primeiro o último vértice do caminho crítico
			...
			for( i = p.topo; i >= 1; i--){				// imprime os vértices do caminho crítico que estão na pilha, indo desde o topo da pilha até o fim da pilha
					fprintf(out, "  %c   %s %2d\n", gr.ver[p.vet[i]].letra, gr.ver[p.vet[i]].desc, gr.ver[p.vet[i]].prazo); // cada duração tem no máximo 2 dígitos, ou seja, no máximo 99 semanas
			}
			esvaziarPilha(&p);							// após usarmos a pilha, ela é esvaziada
		
			fprintf(out, "                                   ----");
			fprintf(out, "\nTEMPO MINIMO PARA O PROJETO:       %4d semanas\n", max); // 50 (número máximo de tarefas) * 99 (duração máxima) = 4950 (tempo máximo do projeto não passa de 4 dígitos)			
		}
		else {// caso não haja vértices no grafo, o tempo mínimo é zero
			fprintf(out, "                                   ----");
			fprintf(out, "\nTEMPO MINIMO PARA O PROJETO:       %4d semanas\n", 0); // 50 (número máximo de tarefas) * 99 (duração máxima) = 4950 (tempo máximo do projeto não passa de 4 dígitos)
		}
	}
...
int hasCycle (struct grafo *gr, struct pilha *p, int *begin, struct pilha *s){	
	int 	    i;								// variável auxiliar para loop
	if (gr->v == 0) return 0;					// grafo sem vértices não tem ciclo
	for (i = 1; i <= gr->v; i++)
