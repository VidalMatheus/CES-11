...
int contaFilhosUnicos(struct noh*x);
	...
	while (strcmp(action,"FIM") != 0){
    	...
    	if (!strcmp (action, "FILHOSUNICOS")){
    		...
    		if (aux == (struct noh*)NULL){
    			...
    			printf("Quantidade de filhos unicos: %d", contaFilhosUnicos(aux) );
		}
...
int contaFilhosUnicos(struct noh* x){
	struct noh*p = (struct noh*) NULL;
	int n = 0;
	if(x == (struct noh*) NULL) return 0; // Caso foi chamada a função para contar filhos únicos folhas
	if(x->pai != (struct noh*) NULL){
		if( x->pai->fesq == x && x->idir == (struct noh*) NULL){
			n++;
			printf("Eh filho unico: %s\n",x->name);
		}
	}
	p = x->fesq;
	while (p != (struct noh*) NULL){
		n+=contaFilhosUnicos(p);
		p=p->idir;
	}
	return n;
}
