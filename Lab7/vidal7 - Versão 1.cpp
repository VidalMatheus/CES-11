...
void printMST(int parent[], struct grafo gr, FILE *out);			// Função auxiliar para escrever no arquivo de saída o MST construído guardado no vetor de pais parent[] 
...
int busca (struct grafo gr, char predio[]){							// função que retorna a posição de um prédio no vetor de posições
	int i;
	for (i = 1; i <= 150 && strcmp(gr.name[i],predio) != 0; i++);		// percorre o vetore de posições até achar a posição do elemento ou até o final
	if ( i == 151){		// caso não tenha encontrado o elemento, retorna -1
		return -1;
	}
...
void printMST(int parent[], struct grafo gr, FILE *out)
{	
	...
	for (i = 2; i <= gr.v; i++){																			// não é necessário escrever o primeiro, já que não tem pai	
		fprintf(out, "%-11s%-11s%3d\n", gr.name[parent[i]], gr.name[i], gr.custo[i][parent[i]]);			// escreve no arquivo de saída em ordem ordem de selecao pelo algoritmo de Prim						
